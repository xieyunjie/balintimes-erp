...
define([ 'angular', 'balintimesConstant' ], function(angular, balintimesConstant) {
			...
			link : function(scope, element) {
				...
				scope.$watch("userMenus", function(menus) {
					...
					for (var i = 0; i < menus.length; i++) {
						menu = menus[i];
						html_menu += '<li class="treeview"><a href="#"><i class="fa ' + menu.iconclass + '"></i> ' + '<span>' + menu.name + '</span><i class="fa fa-angle-left pull-right"></i></a><ul class="treeview-menu">';
				...
				var gentree = function(menu) {
					...
					for (var i = 0; i < menu.children.length; i++) {
						...
						if (child.children.length == 0) {
							...
							html += '<li><a href="#"><i class="fa ' + child.iconclass + '"></i>' + child.name + '<i class="fa fa-angle-left pull-right"></i></a>' + '<ul class="treeview-menu">';
							html += gentree(child);
							html += '</ul></li>';
						}
					}
	...
	appDirective.directive('node', function($compile) {
		return {
			...
			link : function(scope, element) {
				/*
				 * Here we are checking that if current node has children then
				 * compiling/rendering children.
				 */
	...
	// -------------------------------------------------------------------------------------------
	/*
	 * 只选中对应节点,不会选中父节点、子节点的checktree
	 * */
	appDirective.directive('nodepostTree', function() {		
		return {
			template : '<nodepost ng-repeat="node in tree"></nodepost>',
			replace : true,
			restrict : 'E',
			scope : {
				tree : '=children'
	...
	appDirective.directive('nodepost', function($compile) {
		return {
			restrict : 'E',
			replace : true,
			templateUrl : balintimesConstant.rootpath + '/views/tpls/directive/checktreeforpost.html',
			link : function(scope, element) {
				/*
				 * Here we are checking that if current node has children then
				 * compiling/rendering children.
				...
				if (scope.node && scope.node.children && scope.node.children.length > 0) {
					...
					var html = '<ul class="tree" ng-if="node.childrenVisibility"><nodepost-tree children="node.children" text="' + element.attr("text") + '" expanded="' + e + '" checkchildren="' + hasCheckChildren + '"></nodepost-tree></ul>';
