 ...
 *
 * ---VIEWS---
 * A View in UI-Router consists of a com.balintimes.erp.center.controller and a template.  Each view belongs to one state, and a state can have many
 ...
 * initialize the com.balintimes.erp.center.controller, etc.  The ui-view directives caches the locals in a closure variable.  If the locals are
...
// Grab a copy of the $stickyState com.balintimes.erp.center.service for use by the transition management code
...
angular.module("ct.ui.router.extras.sticky").config(
  [ "$provide", "$stateProvider", '$stickyStateProvider', '$urlMatcherFactoryProvider', 'uirextras_coreProvider',
    function ($provide, $stateProvider, $stickyStateProvider, $urlMatcherFactoryProvider, uirextras_coreProvider) {
      ...
      // Decorate the $state com.balintimes.erp.center.service, so we can decorate the $state.transitionTo() function with sticky state stuff.
...
(function(angular, undefined) {
  ...
  function _futureStateProvider($stateProvider, $urlRouterProvider, $urlMatcherFactory, uirextras_coreProvider) {
    ...
    //        com.balintimes.erp.center.controller: function() {
  ...
  // inject $futureState so the com.balintimes.erp.center.service gets initialized via $get();
...
angular.module("ct.ui.router.extras.transition", [ 'ct.ui.router.extras.core' ]).config( [ "$provide",  function ($provide) {
      // Decorate the $state com.balintimes.erp.center.service, so we can replace $state.transitionTo()
